...
class RMQSynchronizedMutableDictionaryTest: XCTestCase {
    ...
    func testMultiThreadedWriting() {
        ...
        DispatchQueue.global(priority: DispatchQueue.GlobalQueuePriority.low).async(group: dictGroup) {
            for n in 0...999 {
                sharedDictionary[n] = values[n]
            }
        }

        DispatchQueue.global(priority: DispatchQueue.GlobalQueuePriority.high).async(group: dictGroup) {
            for n in 1000...1999 {
                sharedDictionary[n] = values[n]
            }
        }

        DispatchQueue.global(priority: DispatchQueue.GlobalQueuePriority.background).async(group: dictGroup) {
            for n in 2000...2999 {
                sharedDictionary[n] = values[n]
            }
        }

        dictGroup.wait(timeout: DispatchTime.distantFuture)
