...
class ChannelSpy : NSObject, RMQChannel {
    ...
    init(aChannelNumber: Int) {
		channelNumber = NSNumber(value: aChannelNumber)
    }
    ...
    func afterConfirmed(_ timeout: NSNumber, handler: @escaping (Set<NSNumber>, Set<NSNumber>) -> Void) -> Void {
    }

    func afterConfirmed(_ handler: @escaping (Set<NSNumber>, Set<NSNumber>) -> Void) -> Void {
    ...
    func basicPublish(_ body: Data, routingKey: String, exchange: String, properties: [RMQValue], options: RMQBasicPublishOptions) -> NSNumber {
        lastReceivedBasicPublishMessage = body as NSData?
		...
		return NSNumber(value: publishReturn);
    }
	...
	//TJ_ADDED:
	func dirent(_ name: String, options: RMQExchangeDeclareOptions) -> RMQExchange {
		return RMQExchange(name: name, type: "direct", options: [], channel: self)
	}
	func dirent(_ name: String) -> RMQExchange {
		return dirent(name, options: [])
	}
	///TJ
