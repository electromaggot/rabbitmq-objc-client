...
class ChannelAllocationTest: XCTestCase {
    ...
    func testNumbersAreNotDoubleAllocated() {
        ...
        let queues      = [
            DispatchQueue.global(priority: DispatchQueue.GlobalQueuePriority.high),
            DispatchQueue.global(priority: DispatchQueue.GlobalQueuePriority.low),
            DispatchQueue.global(priority: DispatchQueue.GlobalQueuePriority.background)
        ]
        ...
        XCTAssertEqual(DispatchTimeoutResult.success, group.wait(timeout: TestHelper.dispatchTimeFromNow(10)), "Timed out waiting for allocations")
    ...
    func testChannelsAreReleasedWithThreadSafety() {
        ...
        XCTAssertEqual(DispatchTimeoutResult.success, group.wait(timeout: TestHelper.dispatchTimeFromNow(10)), "Timed out waiting for releases")
        XCTAssertEqual(1, allocator?.allocate().channelNumber)
    }
