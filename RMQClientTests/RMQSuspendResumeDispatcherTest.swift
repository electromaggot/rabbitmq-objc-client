...
class RMQSuspendResumeDispatcherTest: XCTestCase {
    ...
    func testDisableSuspendsCommandQueueAndPreventsFramesetHandlingFromResuming() {
        ...
        let dispatcher = RMQSuspendResumeDispatcher(sender: SenderSpy(), command: q, enablementQueue: eq, enableDelay: 3)

        dispatcher?.disable()
        XCTAssertTrue(q.suspended)

		(dispatcher as! RMQFrameHandler).handle(RMQFrameset(channelNumber: 1, method: MethodFixtures.basicQosOk()))
        dispatcher?.enable()
    ...
    func testSyncMethodFailureSendsErrorToDelegate() {
		...
		(dispatcher as RMQFrameHandler).handle(RMQFrameset(channelNumber: 123, method: MethodFixtures.basicQosOk()))
        try! q.step()
    ...
    func testBlockingSyncMethodsSentToSender() {
		...
		(dispatcher as RMQFrameHandler).handle(RMQFrameset(channelNumber: 123, method: MethodFixtures.basicGetOk(routingKey: "foo")))
        try! q.step()
    ...
    func testFutureBlockingWaitOnProducesErrorAfterClientClose() {
		...
		(dispatcher as RMQFrameHandler).handle(RMQFrameset(channelNumber: 123, method: MethodFixtures.channelCloseOk()))
        try! q.step()
    ...
    func testCloseOkStopsFutureSyncCompletionHandlersFromExecuting() {
        ...
        (dispatcher as RMQFrameHandler).handle(RMQFrameset(channelNumber: 123, method: MethodFixtures.basicConsumeOk("")))
    ...
    func testCloseDoesNotCauseErrorIfNotTheFirstOperation() {
        ...
        try! q.step()
        (dispatcher as RMQFrameHandler).handle(RMQFrameset(channelNumber: 123, method: MethodFixtures.channelOpenOk()))
        try! q.step()
    ...
    func testServerCloseCausesCloseOkToBeSentInResponse() {
        ...
        (dispatcher as RMQFrameHandler).handle(RMQFrameset(channelNumber: 123, method: MethodFixtures.channelClose()))
    ...
    func testServerCloseSendsErrorToDelegateWithCloseReasonWhen404() {
        ...
        (dispatcher as RMQFrameHandler).handle(RMQFrameset(channelNumber: 123, method: close))
