...
class EncodingTest: XCTestCase {
    ...
    func testEncodeContentHeaderPayload() {
        ...
        let payload = RMQContentHeader(
			classID: NSNumber(value: classID.integerValue),
			bodySize: NSNumber(value: Int(bodySize.integerValue)),
    ...
    func testOptionsBecomeBitfieldOctet() {
		...
		let actual: NSData = method.amqEncoded() as NSData
		let optionsByte = actual.subdata(with: NSMakeRange(actual.length - RMQTable([:]).amqEncoded().count - 1, 1))
